...
import { TrendingUp, Clock, Calendar, ArrowRight, User, Building2 } from 'lucide-react';
...
const caseStudies = [
    {
        ...
        city: "Tirana, AL",
        before: [
            "Missed ~15 calls/week",
            "2h daily manual replies",
            "No after-hours support"
        ],
        after: [
            "Auto-booked 12 appts",
            "0 missed inquiries",
        ...
        impactDecimals: 1,
        impactType: "revenue", // revenue, time, bookings
    ...
    },
    {
        ...
        after: [
            "90% FAQs automated",
            "Pre-vetted consultations",
            "Instant doc collection"
        ],
        impactValue: 3.2,
        impactPrefix: "+$",
        ...
        <section className="py-24 bg-dark relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="text-center mb-16">
                    ...
                    >
                        Real Results From <span className="text-primary">Local Businesses</span>
                    </motion.h2>
                    <motion.p
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
...
function ResultCard({ study, index }: { study: any, index: number }) {
    ...
    return (
        ...
        >
            ...
            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-8 relative">
                ...
                <div className="space-y-3">
                    ...
                    <ul className="space-y-3">
                        {study.before.map((item: string, i: number) => (
                ...
                {/* After Column - Visual Emphasis on Hover */}
                <div className="space-y-3 relative">
                    {/* Connecting Arrow (Mobile hidden, Desktop visible absolute) */}
                    ...
                    <div className="text-xs text-primary uppercase tracking-widest font-bold mb-2 flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_rgba(var(--primary),0.5)]"></span>
                        After 30 Days
                    </div>
                    <ul className="space-y-3">
                        {study.after.map((item: string, i: number) => (
                            <li key={i} className="text-sm text-white font-medium flex items-start gap-2 group-hover:text-primary/90 transition-colors duration-300">
            ...
            <div className="px-6 py-5 bg-gradient-to-r from-primary/5 to-transparent border-t border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                {/* Impact Metric */}
                <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${isRevenue ? 'bg-emerald-500/10 text-emerald-400' : isTime ? 'bg-blue-500/10 text-blue-400' : 'bg-primary/10 text-primary'}`}>
                        {isRevenue ? <TrendingUp className="w-5 h-5" /> : isTime ? <Clock className="w-5 h-5" /> : <Calendar className="w-5 h-5" />}
                    </div>
                    <div>
                        <div className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Impact</div>
                        <div className={`text-lg font-bold ${isRevenue ? 'text-emerald-400' : isTime ? 'text-blue-400' : 'text-primary'}`}>
                            <CountUp value={study.impactValue} prefix={study.impactPrefix} suffix={study.impactSuffix} decimals={study.impactDecimals} duration={0.8} />
                        </div>
                    </div>
                </div>
