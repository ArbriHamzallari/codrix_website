...
export interface RobotHeroProps {
  ...
  metalness?: number;
  /**
   * Whether to render the built-in `AntennaNavbar` overlay. Default true so
   * the component stays usable standalone. Set false when mounting inside a
   * page that already renders its own site-wide nav (e.g. this homepage's
   * `Navbar.tsx` in `layout.tsx`) — otherwise two navbars stack.
   */
  showNavbar?: boolean;
  /**
   * Real hero copy (headline, subtitle, CTAs) rendered bottom-aligned over
   * the 3D scene. The upstream template left this slot empty (just the
   * faint background wordmark) — Biseda's homepage needs the actual value
   * proposition here, so the call site supplies it.
   */
  children?: React.ReactNode;
}
...
export function RobotHero({
  ...
  return (
    <section
      ref={containerRef}
      className="relative w-full h-auto min-h-[1000px] sm:min-h-[960px] lg:h-dvh lg:min-h-[600px] overflow-hidden"
      style={{
        background: `radial-gradient(ellipse 900px 700px at 72% 45%, rgba(109,53,242,0.09) 0%, rgba(109,53,242,0.03) 45%, transparent 70%), linear-gradient(to bottom, #FDFCFF 0%, #F7F6FB 100%)`,
      }}
    >
      {/* Barely-there brand wordmark, tucked behind where the robot sits.
          Hidden below lg — on mobile/tablet there isn't room for it to stay
          out of the headline's way, so it's simplest and safest to drop it
          rather than fight for space. */}
      <div
        className="absolute inset-0 items-center justify-end pointer-events-none overflow-hidden pr-[2%] hidden lg:flex"
        style={{ zIndex: 0 }}
      >
        <h1
      ...
      {/* Soft ambient glow behind the robot — sits between the page
          background and the canvas, doesn't touch the robot's own render. */}
      <div
        className="absolute left-1/2 -translate-x-1/2 bottom-[40px] w-[260px] h-[260px] sm:bottom-[60px] sm:w-[300px] sm:h-[300px] lg:left-auto lg:right-[8%] lg:top-1/2 lg:bottom-auto lg:-translate-y-1/2 lg:translate-x-0 lg:w-[480px] lg:h-[480px] rounded-full bg-primary/10 lg:bg-primary/15 blur-[90px] lg:blur-[110px] pointer-events-none z-[5]"
        aria-hidden
      />

      {/* Canvas gets its own region at the bottom of the section below lg —
          the section grows tall enough (see className above) for the
          headline/paragraph/CTA block to sit above it without the robot
          overlapping the text. At lg+ this reverts to the original
          full-bleed layout, robot vertically centered behind the copy. */}
      <div className="absolute inset-x-0 bottom-0 h-[420px] sm:h-[460px] lg:inset-0 lg:h-auto z-10">
      ...
      <div className="absolute inset-0 z-20 pointer-events-none flex flex-col">
        ...
        <div className="relative w-full max-w-[1400px] mx-auto px-8 flex-1 flex flex-col justify-start pt-28 lg:justify-center lg:pt-0">
