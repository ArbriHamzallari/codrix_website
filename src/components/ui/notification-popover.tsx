...
interface NotificationPopoverProps {
  ...
  /** Unread-dot colour. Defaults to `bg-white`, as before — light themes need
   *  to override it or the dot vanishes into the popover background. */
  dotColor?: string;
  /** Bell glyph size in px. Defaults to 16, as before. */
  iconSize?: number;
  /** Accessible name for the icon-only trigger. */
  buttonAriaLabel?: string;
  /** Popover header label. Defaults to "Notifications" (this site is
   *  Albanian-first — see CLAUDE.md §7 — so call sites pass a localized one). */
  title?: string;
  /** "Mark all as read" label, localized the same way. */
  markAllLabel?: string;
  /** How each timestamp renders. Defaults to `toLocaleDateString()`, exactly as
   *  before; call sites can pass relative times ("tani", "12 min") instead. */
  formatTimestamp?: (date: Date) => string;
  /** Controlled-open escape hatch, so a section can open the popover on
   *  scroll-into-view. Omit for the original self-managed behaviour. */
  open?: boolean;
