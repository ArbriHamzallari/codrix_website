# Robot Hero — Integration Prompt for Claude Code

Paste this whole file into Claude Code, running inside the `codrix_website` repo.

## Context — read this first

- Repo: `codrix_website` — Next.js 16 (App Router), React 19, TypeScript, Tailwind CSS v4.
- There is **no shadcn CLI / `components.json`** in this repo. Don't try to run `shadcn add` or bootstrap shadcn — it's not used here.
- There **is** an established convention of a `src/components/ui/` folder for standalone UI pieces (see `src/components/ui/CountUp.tsx`). Use that folder for this component.
- `framer-motion` (`^12.23.26`) is already a dependency — no version change needed.
- **Important:** local uncommitted changes may already exist in this repo (brand colors, copy, etc.) that aren't reflected in git history. Before writing anything, run `git status` and `git diff`, and read the current contents of `tailwind.config.mjs`, `src/components/Hero.tsx`, and `src/app/page.tsx` so you're building on top of what's actually there right now, not an assumed clean state.

## Task

Add the `RobotHero` component below to the codebase and mount it as the new homepage hero, replacing the current industry-selector `Hero.tsx` section rendered from `src/app/page.tsx`.

## 1. Install dependencies

```bash
npm install three @react-three/fiber @react-three/drei react-icons
```

## 2. Create the component — do not alter the 3D geometry, shaders, or animation logic

Create `src/components/ui/robot-hero.tsx` with exactly this content:
...
## 3. What was changed vs. the original template, and why

These are functional wiring fixes, not brand/color decisions — the geometry, materials, and animation are byte-for-byte the same:

- `backgroundText` default: `"UITHEFACTORY"` → `"BISEDA AI"` (it was literally the template author's name).
- `navItemsLeft` default: the placeholder `Product / About / Specs / Reviews` now point at real sections/pages that already exist in this repo (`#how-it-works`, `/ai-agents`, `/crm`, `/portfolio`). Swap these for whatever the finalized site IA ends up being.
- `ctaText` default: `"Buy Now"` → `"Book a Demo"`, and `contactHref` defaults to `#book`, matching the `scrollToId('book')` pattern already used in `Hero.tsx`. The shopping-bag icon (`PiShoppingBagBold`) is still there next to the CTA button in `AntennaNavbar` — that's a leftover from the e-commerce template. Leave it for Arbri to swap or remove once he's decided on final CTA iconography.
- `color` / `pantallaColor` / `metalness` / `blinkCycle`: left as component props, untouched defaults. These are exposed specifically so the Biseda purple/violet values (or whatever's already set locally) get passed in from the call site in step 4, not hardcoded into the component.

## 4. Replace the homepage hero mount

Find where `<Hero />` (from `src/components/Hero.tsx`) is currently rendered — almost certainly `src/app/page.tsx`. Replace:
...
// ...
<RobotHero
  color="#5B2CFF"       // placeholder — replace with whatever's set locally
  pantallaColor="#A855F7" // placeholder — replace with whatever's set locally
  onCtaClick={() => document.getElementById('book')?.scrollIntoView({ behavior: 'smooth' })}
/>
```

Don't delete `src/components/Hero.tsx` — just stop importing it on the homepage, in case Arbri wants to A/B or revert.

## 5. Before committing

- `npm run build` and check the bundle size warning — `three` + `@react-three/fiber` + `@react-three/drei` is a heavy addition, and this is the very first thing a prospective clinic/salon owner sees, often on mobile.
- Test on a throttled connection. If first paint visibly stutters or delays, say so back to Arbri rather than shipping silently — it runs against the "magic moment in under 3 minutes" doctrine this site is built around.
- Confirm nothing else on the page (e.g. `HeroChatDemo`) breaks from the removal of the old `<Hero />` mount.

## 6. Document it

Add a short entry to `CLAUDE.md` noting: homepage hero swapped from `Hero.tsx` to `RobotHero` (3D, React Three Fiber), plus the new `three` / `@react-three/fiber` / `@react-three/drei` / `react-icons` dependencies — consistent with how this repo already documents changes.